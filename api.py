...
def detectSeeds(image):
    ...
    ret, maskB = cv2.threshold(image[:,:,2], 150, 255, cv2.THRESH_BINARY_INV)
    ...
    mask = cv2.bitwise_and(mask, maskG)
...
def classifySeeds(image):
    ...
    for cnt in good_cnt:
        ...
        if w * h > min_area:
            cnt_germ.append([x, y, w, h])
            geminated = geminated + 1
            cv2.rectangle(image, (x, y), (x + w, y + h), (255, 0, 0), 4)
        else:
            cnt_nongerm.append([x, y, w, h])
